...
class Podcast extends CI_Controller {
	...
	public function index()
	{
        ...
        $total_records  = $this->front_model->get_podcastcount();      
        $limit_per_page = 10;
        if ($total_records > 0)
        {
            // get current page records
            $data["podcast"] = $this->front_model->get_podcast($limit_per_page, $page*$limit_per_page);	
            
            $config["base_url"] = base_url() . "web/podcast/index/";
        ...
        $this->pagination->initialize($config);        
		$this->load->view('web/podcast',$data);
		$this->load->view('web/includes/footer');
	}

    public function podcast_detail($Podcastid){
    
        $data["podcast"] = $this->front_model->get_podcastdetail($Podcastid);
        $data["page_title"] = $data["podcast"]["title"];
        $this->load->view('web/includes/header',$data);
        $this->load->view('web/podcast_detail',$data);
        $this->load->view('web/includes/footer');
    }
}