...
class Event extends CI_Controller {
	...
	public function index()
	{
        ...
        $total_records  = $this->front_model->get_eventcount();      
        $limit_per_page = 10;
        if ($total_records > 0)
        {
            // get current page records
            $data["event"] = $this->front_model->get_event($limit_per_page, $page*$limit_per_page);	
            
            $config["base_url"] = base_url() . "web/event/index/";
     ...
     public function event_detail($event_id)
    {
        ...
        $this->load->view('web/includes/header',$data);
        $data["event"] = $this->front_model->get_eventdetail($event_id);
        $this->load->view('web/event_detail',$data);
        $this->load->view('web/includes/footer');
    }
}