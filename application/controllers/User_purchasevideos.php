...
class User_purchasevideos extends CI_Controller {
	...
	public function index(){
		...
		$data["title"] = "User Purchase Video";

		$data["heding"] = "User Purchase Video";

		$this->load->view('includes/header',$data);

		$cutmer_data = $this->db->query(" select customer.id,full_name,swashoppe_master.id as swashoppe_videoid,title,thumbnail_image,video,video_tracking.id as video_tracking_id,user_id,video_id,total_duration,strat_time,pause_time,watching_duration from customer join video_tracking on customer.id = video_tracking.user_id join swashoppe_master on video_tracking.video_id = swashoppe_master.id where video_tracking.pause_time !=0 and video_tracking.strat_time !=0  GROUP BY video_tracking.user_id order by video_tracking.id DESC ")->result_array();

		$data["user_videos"] =$cutmer_data;

		$this->load->view('user_purchasevideos/user_purchasevideos_list',$data);
